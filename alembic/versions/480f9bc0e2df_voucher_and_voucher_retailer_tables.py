...
def upgrade():
    ...
    op.create_foreign_key(None, "voucher_config", "voucher_retailer", ["voucher_retailer_id"], ["id"])
    op.drop_column("voucher_config", "retailer_slug")
    op.execute("INSERT INTO voucher_retailer (retailer_slug) VALUES('test-slug')")
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column(
        "voucher_config", sa.Column("retailer_slug", sa.VARCHAR(length=32), autoincrement=False, nullable=False)
    )
    op.drop_constraint(None, "voucher_config", type_="foreignkey")
